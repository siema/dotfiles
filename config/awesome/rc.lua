...
local function add_tag()
    ...
    awful.tag.add(#screen.tags + 1, { screen = screen }):view_only()
end

local function delete_tag()
    local t = awful.screen.focused().selected_tag
    if (not t) or (t.index <= 6) then return end
    t:delete()
end
...
client.connect_signal("request::titlebars", function(c)
    ...
    if not (awful.layout.get(c.screen).name == "floating" or c.floating) then
        awful.titlebar.hide(c)
    end
    -- if (not (awful.layout.get(c.screen).name == "floating" or c.floating)) or (awful.layout.get(c.screen).name == "max" or c.maximized) then
    --     awful.titlebar.hide(c)
    -- end
end)
